num_rows = 50;%64; 
num_cols = 50;%64;
num_angles = 50;%19;%180; 
detectors = 50;%2*round(num_cols * num_rows / num_angles);%128;
...
for angle_index = 0 : num_angles - 1 
    ...
    rotated_coords = rotation_matrix * [X_interp(:)';
                                        Y_interp(:)'];
...
while k <= max_iterations%reconstruction_error > 0.1
    for angles = 0 : num_angles - 1
        for ray = 1 : detectors
            ...
            lambda = 1 / max(sum(system_matrix, 2));
            if (length == 0)
            ...
            previous_image = previous_image + lambda * transpose(system_matrix(index, :)) * normalised_error;
        end
    end
    ...
    reconstruction_error = immse(system_matrix * previous_image, Radon);%norm(Radon - system_matrix * previous_image) / norm(Radon);

    reconstruction_error2 = [reconstruction_error2, reconstruction_error];
    disp(['Iteration ', num2str(k), ' error: ', num2str(reconstruction_error)]);
    k = k + 1;
end
reconstruction_error2 = reconstruction_error2 / max(reconstruction_error2, [], "all");
...
title('Reconstructed Image with ART');