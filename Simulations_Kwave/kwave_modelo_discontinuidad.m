...
%===================
    ...
    z= Nx/2-80:6:Nx/2+80; %round(3/(1000*dx))
    ...
    % Visualizar el campo de presion de la señal adquirida
    % figure;
    ...
    % title(['Rotación de Sensores y Fuentes en Ángulo: ', num2str(Theta)]);
    % xlabel('y-position [mm]');
    % ylabel('x-position [mm]');
    % title('Final Wave Field');
    % % legend('E_{R_s}', 'E_{T_s}');
    % % %ylim([-.1, 1.1])
    % % grid on;
    % hold off;
end
...
for a=1:length(ang)
    ...
    for i=1+(a-1)*length(z):length(z)*a
        [amp(k,a), indice]=max(signals(:,i));%hilbert(abs(signals(:,i))));
        delta_t(k,a) = kgrid.t_array(indice);
       
        Energy(k, a) = (1/L) * trapz(kgrid.t_array, signals(:, i).^2);%hilbert(abs(signals(:, i))).^2);
...
reconstruction2 = iradon(N_energy, Phi,"Ram-Lak");%delta_tflip("Ram-Lak");%
